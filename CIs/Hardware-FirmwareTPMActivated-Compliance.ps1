...
function Get-DellTPMActiveStatus {
    [CmdLetBinding()]
    Param(
        [Parameter(
            ValueFromPipeline=$True,
    ...
    Process {
        ...
        $TPMSetting = Get-WmiObject -ComputerName $ComputerName -Namespace root\dcim\sysman -ClassName DCIM_BiosEnumeration | Where-Object {$_.AttributeName -eq 'Trusted Platform Module Activation'}
...
If ((Get-DellBootMode).BootMode -eq 'UEFI') {
    ...
    If ((Get-DellTPMActiveStatus).TPMActivated) {

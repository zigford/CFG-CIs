...
logMsg "Remove old start layout"
Try {
    Remove-Item -Path "$($env:SystemDrive)\Users\default\AppData\Local\Microsoft\Windows\Shell\LayoutModification.xml"
    logMsg "Succesfully removed old layout"
} catch {
    logMsg "Couldn't remove old layout" 2
}
